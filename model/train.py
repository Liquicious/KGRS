...
def start(args, data, show_topk, show_titles):
    n_user, n_item, n_entity, n_relation = data[0], data[1], data[2], data[3]
    train_data, eval_data, test_data = data[4], data[5], data[6]
    adj_entity, adj_relation = data[7], data[8]

    print('n users:', n_user)
    print('n items:', n_item)
    print('n entities:', n_entity)
    print('n relations:', n_relation)
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for step in range(args.n_epochs):
            ...
            while start + args.batch_size <= train_data.shape[0]:
                _, __ = model.train(sess, get_feed_dict(model, train_data, start, start + args.batch_size))
...
def train(args, data, show_loss, show_topk, show_titles):
    n_user, n_item, n_entity, n_relation = data[0], data[1], data[2], data[3]
    train_data, eval_data, test_data = data[4], data[5], data[6]
    adj_entity, adj_relation = data[7], data[8]
...
def show_user_predictions(sess, model, user, train_record, item_set, batch_size, show_titles, dataset_name):
    test_item_list = list(item_set - train_record[user])
    item_score_map = dict()
    start = 0
    while start + batch_size <= len(test_item_list):
    ...
    user_records = list(train_record[user])
    print('user record:')

    if show_titles:
        titles = load_titles('C:/Users/korol/PycharmProjects/KGRS-j/sets/'
                             + dataset_name + '/ids_with_names.txt')

        for i in user_records:
            print(titles[i])

        print('recommendations:')
        for x in item_score_pair_sorted[:10]:
            print(titles[x[0]] + ':', x[1])
    else:
        for i in user_records:
            print(i)
